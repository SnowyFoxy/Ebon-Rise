    ...
    public class JobGiver_Escape : ThinkNode_JobGiver
    {
        ...
        private IntVec3? FindEscapeCell(Pawn pawn)
        {
            ...
            if (!escapeCell.IsValid)
                return null;
            using PawnPath path1 = pawn.Map.pathFinder.FindPathNow(pawn.Position, escapeCell, TraverseParms.For(pawn, mode: TraverseMode.PassDoors));
            if (path1.Found) return escapeCell;
            using PawnPath path2 = pawn.Map.pathFinder.FindPathNow(pawn.Position, escapeCell, TraverseParms.For(pawn, mode: TraverseMode.PassAllDestroyableThings));
